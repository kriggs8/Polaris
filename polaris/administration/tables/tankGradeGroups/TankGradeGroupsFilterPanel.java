...
public class TankGradeGroupsFilterPanel extends GenericFilterPanel
{
    ...
    public TankGradeGroupsFilterPanel(MainPanel mainPanel) throws Exception
    {
        ...
        tankGradeGroupCombo = ComponentFactory
                .getComboBox(true, "TANK_GRADE_GROUP_CONF", TankGradeGroupsConstants.tankGradeGroupDBWhere0,
                        TankGradeGroupsConstants.tankGradeGroupDBName, SpecialItems.ALL);
        ...
        scheduleSystemCombo = ComponentFactory.getScheduleSystemComboBox(TankGradeGroupsConstants.schedSystemDBWhere0,
                TankGradeGroupsConstants.schedSystemDBName, SpecialItems.ALL);
        //Set up System combo box
        systemCombo = ComponentFactory
                .getSystemComboBox(TankGradeGroupsConstants.systemDBWhere0, TankGradeGroupsConstants.systemDBName,
                        SpecialItems.ALL);
    ...
    public String getWhereClause()
    {
        ...
        if ((where.contains(TankGradeGroupsConstants.systemDBName) ||
             where.contains(TankGradeGroupsConstants.schedSystemDBName)) &&
            where.contains(TankGradeGroupsConstants.gradeDBName))
        {
        ...
        } else if (where.contains(TankGradeGroupsConstants.systemDBName) ||
                   where.contains(TankGradeGroupsConstants.schedSystemDBName))
        {
        ...
        if (e.getSource() instanceof JComboBox)
        {
           ...
           // update for system combo box
            ...
            if (action != gradeCombo.getName() && !gradeCombo.hasFocus() ||
                gradeCombo.hasFocus() && gradeCombo.getSelectedItem() != null &&
                gradeCombo.getSelectedItem().equals(SpecialItems.ALL))
            {
                if ((where.contains(TankGradeGroupsConstants.systemDBName) ||
                     where.contains(TankGradeGroupsConstants.schedSystemDBName)) &&
                    where.contains(TankGradeGroupsConstants.tankGradeGroupDBName))
